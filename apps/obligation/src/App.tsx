...
import { ObligationApplicationProvider } from "./obligation-context";
 ...
 * compiled application IR. Nothing here names an obligation field or status:
...
export function App({
  ...
  return (
    <OUIThemeProvider density={density} theme={theme}>
      <section className="obligation-controls" aria-label="Appearance">
        <Inline>
          <Selection
            label="Theme"
      ...
      <OUIApplicationRoot bootstrap={composition.applicationBootstrap}>
        <ObligationApplicationProvider composition={composition}>
          <OUIRouterProvider runtime={routerRuntime} />
        </ObligationApplicationProvider>
      </OUIApplicationRoot>
    </OUIThemeProvider>
  );
}

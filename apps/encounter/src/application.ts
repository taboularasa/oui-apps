 ...
 */
export const encounterCapabilities = Object.freeze([
...
export interface EncounterApplicationComposition {
...
export function createEncounterApplicationComposition(options: {
...
}): EncounterApplicationComposition {
  ...
  if (applicationBootstrap.status !== "ready") {
    throw new Error(
      `The compiled encounter application IR failed validation: ${JSON.stringify(
  ...
  clients.register({
    serviceBindingId: "service:encounter",
  ...
  return Object.freeze({
    applicationBootstrap,
    operationRuntime,
    extensionCatalog: encounterExtensionCatalog,
  });
}
...
export const encounterExtensionCatalog = extensionRegistry.seal("1.0.0");

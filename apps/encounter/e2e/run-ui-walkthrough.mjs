// Walks the rendered encounter workbench in Chromium against a live generated
...
const origin = process.env.ENCOUNTER_UAT_ORIGIN ?? "http://127.0.0.1:18090";
const token = process.env.ENCOUNTER_UAT_TOKEN;
if (!token) {
  console.error("ENCOUNTER_UAT_TOKEN is required");
  process.exit(2);
}
...
await page.waitForURL(/\/uat\/encounters/u, { timeout: 60_000 });
...
record(
  "collection renders the tenant's encounters",
  rowCount === 4,
  `${rowCount} rows`,
);
...
record(
  ...
  detailHref !== null && /\/uat\/encounters\/encounter-\d/u.test(detailHref),
  `href = ${detailHref}`,
);

await scheduled.click();
await page.waitForURL(/\/uat\/encounters\/encounter-1/u, { timeout: 60_000 });
await page.waitForTimeout(1_500);
const detailText = await page.locator("main, body").first().innerText();
record(
  "detail route renders the selected encounter",
...
await page.screenshot({ path: "e2e/encounter-detail.png", fullPage: true });
...
record(
  ...
  page.url().includes("/uat/encounters/encounter-1"),
  page.url(),
);
record(
...
await page.goto(`${origin}/uat/encounters/encounter-1/edit`, {
  waitUntil: "domcontentloaded",
});
await page.waitForTimeout(2_500);
...
// --- editing actually advances the encounter ------------------------------
await page.goto(`${origin}/uat/encounters/encounter-2/edit`, {
  waitUntil: "domcontentloaded",
});
await page.waitForTimeout(2_500);
...
record(
  ...
  /\/uat\/encounters\/encounter-2$/u.test(page.url()),
  page.url(),
);
...
await page.goto(`${origin}/uat/encounters/new`, {
  waitUntil: "domcontentloaded",
});
await page.waitForTimeout(2_500);
const createText = await page.locator("body").innerText();
record(
  "the create route renders a form for a new encounter",
